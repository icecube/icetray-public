...
I3DOMCalibration::I3DOMCalibration()
  : temperature_(NAN),
    ...
    meanATWDChargeCorrection_(NAN),
    meanFADCChargeCorrection_(NAN)
{
  ...
  if (version > 10)
    {
      ...
      ar & make_nvp("meanATWDCharge", meanATWDChargeCorrection_);
      ar & make_nvp("meanFADCCharge", meanFADCChargeCorrection_);
    }
  if (version == 11)
    {
      // these only existed in version 11, replaced with
      // meanATWDCharge_/meanFADCCharge_ == NaN in v12
      bool meanATWDChargeCorrectionValid(false);
      bool meanFADCChargeCorrectionValid(false);
      ar & make_nvp("meanATWDChargeValid", meanATWDChargeCorrectionValid);
      ar & make_nvp("meanFADCChargeValid", meanFADCChargeCorrectionValid);

      if (!meanATWDChargeCorrectionValid) meanATWDChargeCorrection_=NAN;
      if (!meanFADCChargeCorrectionValid) meanFADCChargeCorrection_=NAN;
    }
  if (version > 12)
    {
...
std::ostream& operator<<(std::ostream& oss, const I3DOMCalibration& c)
{
  oss << "[ I3DOMCalibration  :: " << std::endl
      ...
      << " MeanATWDChargeCorrection : " << c.GetMeanATWDChargeCorrection() << std::endl
      << " MeanFADCChargeCorrection : " << c.GetMeanFADCChargeCorrection() << std::endl
      << "]" ;
  return oss;
}
