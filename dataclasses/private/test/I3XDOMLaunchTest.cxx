...
std::vector<int> setLaunchA(I3XDOMLaunch& launch, bool lazy=true) {
...
std::vector<int> setLaunchB(I3XDOMLaunch& launch) {
...
TEST(equal_operator) {
  ...
  I3XDOMLaunch bar;
  setLaunchA(bar);
  ENSURE(foo == bar, "I3XDOMLaunch operator== failed");
  bar.SetLCFlags(UpgradeLCFlags::NoLC);
  ENSURE(!(foo == bar), "I3XDOMLaunch ! operator== failed");
  ENSURE(foo != bar, "I3XDOMLaunch ! operator== failed");
}
...
TEST(serialize) {
  ...
  do {
    ...
    // Build a binary stringstream and serialize the I3XDOMLaunch
    ...
    ENSURE(foo == bar,
          "I3XDOMLaunch operator== failed, launch didn't serialize correctly");
...
TEST(copy_move_constructor) {
  ...
  std::vector<int> adcData = setLaunchA(foo);
  {
    I3XDOMLaunch bar(foo);
    ENSURE(foo == bar, "I3XDOMLaunch copy constructor failed");
  }
  {
    I3XDOMLaunch bar(std::move(foo));
