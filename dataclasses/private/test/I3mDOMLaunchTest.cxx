...
std::vector<int> setLaunchA(I3mDOMLaunch& launch, bool lazy=true) {
  ...
  launch.SetTime(3.*I3Units::microsecond, true)
        ...
        .SetTriggerType(I3mDOMLaunch::TriggerType::ADC_THRESHOLD)
        .GetTOTHits().emplace_back(102u, 11u);
  launch.GetTOTHits().emplace_back(217u, 21u);
  return adcData;
}
...
std::vector<int> setLaunchB(I3mDOMLaunch& launch) {
  ...
  launch.SetTime(32.*I3Units::microsecond)
        ...
        .SetTriggerType(I3mDOMLaunch::TriggerType::DISCRIMINATOR)
...
TEST_GROUP(I3mDOMLaunch);

TEST(TOTHit) {
  {
    I3mDOMLaunch::TOTHit foo;
    ENSURE(std::isnan(foo.GetTime()),
           "Default constructor did not set time to NAN");
    ENSURE(std::isnan(foo.GetTOT()),
           "Default constructor did not set TOT to NAN");
    foo.SetTime(12.*I3Units::microsecond);
    ENSURE_EPSILON(foo.GetTime()/I3Units::microsecond, 12., 1e-9,
                   "SetTime did not set time to 12us");
    ENSURE(std::isnan(foo.GetTOT()), "Oups, SetTime also changes TOT");
    foo.SetTOT(45.*I3Units::nanosecond);
    ENSURE_EPSILON(foo.GetTOT()/I3Units::nanosecond, 45., 1e-9,
                   "SetTOT did not set TOT to 45ns");
    ENSURE_EPSILON(foo.GetTime()/I3Units::microsecond, 12., 1e-9,
                   "Oups, SetTOT also changes time");
  }
  {
    I3mDOMLaunch::TOTHit foo(12.*I3Units::microsecond, 45.*I3Units::nanosecond);
    ENSURE_EPSILON(foo.GetTime()/I3Units::microsecond, 12., 1e-9,
                   "Constructor did not set time to 12us");
    ENSURE_EPSILON(foo.GetTOT()/I3Units::nanosecond, 45., 1e-9,
                   "Constructor did not set TOT to 45ns");
  }
  {
    I3mDOMLaunch::TOTHit foo(9000u, 51u, 1000 * I3Units::megahertz);
    ENSURE_EPSILON(foo.GetTime()/I3Units::microsecond, 9., 1e-9,
                   "Constructor did not set time to 9us");
    ENSURE_EPSILON(foo.GetTOT()/I3Units::nanosecond, 51., 1e-9,
                   "Constructor did not set TOT to 51ns");
    I3mDOMLaunch::TOTHit bar(9.*I3Units::microsecond, 51.*I3Units::nanosecond);
    ENSURE_EQUAL(foo, bar, "Oups, these should be equal");
    foo.SetTime(12.*I3Units::microsecond);
    ENSURE(foo != bar, "Oups, these shouldn't be equal");
  }
}
...
TEST(default_constructor) {
  ...
  EXPECT_THROW(foo.GetValidBaselineValue(),
               "Default constructor did not set baseline to 'invalid'");
  ENSURE(foo.GetTOTHits().empty(), "Default constructor must not create TOT data");
  ENSURE_EQUAL(foo.GetTriggerType(), I3mDOMLaunch::TriggerType::UNDEFINED,
               "Default constructor did not set trigger type to 'UNDEFINED'");
}
...
TEST(set) {
  ...
  ENSURE_EQUAL(foo.GetTOTHits().size(), 2u, "Cannot add TOT data to launch");
  ENSURE_EQUAL(foo.GetTriggerType(), I3mDOMLaunch::TriggerType::ADC_THRESHOLD,
               "SetTriggerType did not set trigger type to 'ADC_THRESHOLD'");
}
...
TEST(equal_operator) {
  ...
  I3mDOMLaunch bar;
  setLaunchA(bar);
  ENSURE(foo == bar, "I3mDOMLaunch operator== failed");
  bar.SetTriggerType(I3mDOMLaunch::TriggerType::SOFTWARE);
  ENSURE(!(foo == bar), "I3mDOMLaunch ! operator== failed");
  ENSURE(foo != bar, "I3mDOMLaunch ! operator== failed");
}
...
TEST(serialize) {
  ...
  do {
    ...
    // Build a binary stringstream and serialize the I3mDOMLaunch
    ...
    I3mDOMLaunch bar;
    // Deserialize a second I3mDOMLaunch from the serialized stream for comparison
    ...
    ENSURE(foo == bar,
          "I3mDOMLaunch operator== failed, launch didn't serialize correctly");
...
TEST(copy_move_constructor) {
  ...
  std::vector<int> adcData = setLaunchA(foo);
  {
    I3mDOMLaunch bar(foo);
    ENSURE(foo == bar, "I3mDOMLaunch copy constructor failed");
  }
  {
    I3mDOMLaunch bar(std::move(foo));
    ...
    ENSURE(bar.HasValidBaselineValue(),
           "baseline didn't move construct correctly");
    ENSURE_EQUAL(bar.GetTOTHits().size(), 2u,
                 "TOT data didn't move construct correctly");
    ENSURE_EQUAL(bar.GetTriggerType(), I3mDOMLaunch::TriggerType::ADC_THRESHOLD,
                 "Trigger type didn't move construct correctly");
  }
}
...
TEST(assignment_operator) {
  ...
  ENSURE(bar.HasValidBaselineValue(),
         "baseline didn't move assign correctly");
  ENSURE_EQUAL(bar.GetTOTHits().size(), 2u,
               "TOT data didn't move assign correctly");
  ENSURE_EQUAL(bar.GetTriggerType(), I3mDOMLaunch::TriggerType::ADC_THRESHOLD,
               "Trigger type didn't move assign correctly");
}
