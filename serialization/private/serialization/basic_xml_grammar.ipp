...
struct append_string {
    ...
    #if 0
        typedef icecube::archive::iterators::xml_unescape<Iterator> translator;
        contents.append(
            translator(I3_MAKE_PFTO_WRAPPER(start)),
...
void basic_xml_grammar<CharType>::init(IStream & is){
    ...
    if(! std::equal(rv.class_name.begin(), rv.class_name.end(), I3_ARCHIVE_SIGNATURE()))
        icecube::serialization::throw_exception(
