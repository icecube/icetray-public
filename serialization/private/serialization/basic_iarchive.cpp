...
class basic_iarchive_impl {
    ...
    basic_iarchive_impl(unsigned int flags) :
        m_archive_library_version(I3_ARCHIVE_VERSION()),
        m_flags(flags)
    {}
    ~basic_iarchive_impl(){}
...
basic_iarchive_impl::load_object(
    ...
    icecube::serialization::state_saver<object_id_type> ss_start(m_moveable_objects.start);
...
basic_iarchive_impl::load_pointer(
    ...
    if(class_id_type(cobject_info_set.size()) <= cid){
        ...
        || bpis_ptr->get_basic_serializer().is_polymorphic()){
            ...
            char key[I3_SERIALIZATION_MAX_KEY_SIZE];
...
I3_ARCHIVE_DECL(const basic_pointer_iserializer *)
...
I3_ARCHIVE_DECL(icecube::archive::library_version_type)
basic_iarchive::get_library_version() const{
    return pimpl->m_archive_library_version;
}

I3_ARCHIVE_DECL(unsigned int)
