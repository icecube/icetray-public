...
class sphinx_writer:
    ...
    def file_header(self):
        if opts.title:
            ...
            self.file.write("    This this file was autogenerated via :ref:`icetray-inspect`\n\n")
    ...
    def module_header(self,modname,category,docs):
        ...
        if opts.sphinx_functions:
            ...
            self.indent = " " * 2
            self.file.write(f"* :js:data:`{altname}` *({category})* -")
        else:
            self.file.write("**%s** (%s)\n\n"
                                            %(modname,category))

        if docs:
            self.file.write(fill(' '.join(docs.strip().splitlines()),
                                 width=88,
    ...
    def parameters(self,config):
        if config:
            ...
            for k in config.keys():
                ...
                # strip and squash whitespace
                s = " ".join(desc[k].split()).strip()
                self.file.write(f"    :param {k}: *Default* = ``{default}``,\n")
                if s:
                    self.file.write(fill(f"{s}",
                                         width=88,
                                         initial_indent=" " * 8,
                                         subsequent_indent=" " * 8))
                    self.file.write("\n\n")
