from .Detector import Detector, PulseData

from .GeometryTools import get_radius

import numpy as np

from icecube import icetray
from icecube import dataclasses
from icecube.dataclasses import FFTData3D
from icecube.icetray import *
from icecube.icetray import I3Units
from icecube.dataclasses import I3Constants
from icecube import radcube
# from icecube.radcube import Rad

from matplotlib.collections import PatchCollection
import matplotlib.patches as mpatches
import matplotlib.path as mpath

"""
This class needs to be done.
In order to be completed the radio functions are needed.
Thus, this class will be completed when those functions will be implemented
"""


class Antenna(Detector):
    """docstring for Antenna"""

    def __init__(self):
        super(Antenna, self).__init__()
        self.antennakeys = self.GetDefaultAntennaKeys()
        self.antennas_pulse_patches = PatchCollection([])
        # self.Efield_keys = ["EFieldTimeSeries", "CoREASEFieldMap", "I3AntennaDataMap"]
        # self.Volt_keys = ["TAXIRadioWaveform", "ResampledVoltageMap"]
        self.color = "b"
        self.name = "Antenna"
        self.timeUnit = I3Units.nanosecond
        self.timeUnitName = "ns"
        self.freqUnit = I3Units.megahertz
        self.freqUnitName = "MHz"
        self.voltageUnit = I3Units.volt * 1e-6
        self.voltageUnitName = "$\mu$V"
        self.specUnit = self.voltageUnit / self.freqUnit
        self.specUnitName = self.voltageUnitName + "/" + self.freqUnitName
        self.antenna_lables = []
        self.AntennaStationID = "None"

    def GetDefaultAntennaKeys(self):
        return ["TAXIRadioWaveform", "ResampledVoltageMap", "EFieldTimeSeries", "CoREASEFieldMap", "I3AntennaDataMap"]

    def GetKeyName(self):
        return self.name

    def ExtractFromGFrame(self, frame):
        assert (frame.Stop == icetray.I3Frame.Geometry)

        self.positions.clear()

        # Check all things in geometry frame I3Geometry
        for key in frame.keys():
            i3geometry = frame[key]
            for antkey, ant in i3geometry.antennageo:
                pos = ant.position
                self.positions[str(antkey)] = np.asarray((pos.x, pos.y, pos.z))

    def DrawGeometry(self, ax):
        antenans_pos = []
        antenna_patches = []
        # for pos in self.positions.values():
        #     antenans_pos.append(pos[:2])
        # if not antenans_pos == []:
        #     antenans_pos = np.asarray(antenans_pos)
        #     self.antennas_position_patches = ax.scatter(antenans_pos[:, 0], antenans_pos[:, 1], color=self.color,
        #                                                 marker='+', s=20)
        for pos in self.positions.values():
            antenna_patches.append(self.antennaPatches(pos[:2]))
        self.antennas_position_patches = PatchCollection(antenna_patches, match_original=True)
        ax.add_collection(self.antennas_position_patches)

    def ExtractFromQPFrame(self, frame):
        self.measuredData.clear()

        for framekey in self.antennakeys:
            if framekey in frame.keys() and len(frame[framekey]) != 0:
                ant_map = frame[framekey]
                self.measuredData[framekey] = ant_map
            else:
                print("Antenna key %s not in frame or empty" % (framekey))

    def DrawLDF(self, ax, particle):
        pass

    def DrawShowerFront(self, ax, particle):
        pass

    def GetDrawOptions(self, frame):
        print("Current pulse keys are", self.antennakeys)
        user_response = input("Enter desired keys: ")
        partKeys = user_response.split()

        self.antennakeys = []
        for key in partKeys:
            if key != "":
                self.antennakeys.append(key)

        print(self.name, "keys set to", self.antennakeys)

    def GetAntennaLabels(self, frame):
        antenna_lables = []
        for key in self.antennakeys:
            if key in frame.keys():
                for antkey in frame[key].keys():
                    if not str([antkey.GetAntennaID(), antkey.GetStationID()]) in antenna_lables:
                        antenna_lables.append(str([antkey.GetAntennaID(), antkey.GetStationID()]))
        self.antenna_lables = antenna_lables

    def AntennaOnClick(self, click_pos, frame, axlist):
        if not self.positions == {}:
            antennaKeys = []
            antennaPos = []
            for antKey in self.positions.keys():
                antennaKeys.append(antKey)
            for pos in self.positions.values():
                antennaPos.append(pos[:2])
            antennaPos = np.asarray(antennaPos)
            antennaKeys = np.asarray(antennaKeys)
            distance = np.sum((antennaPos - click_pos) ** 2, axis=1)
            _min = np.argmin(distance)
            self.AntennaStationID = antennaKeys[_min]
            self.__fill_text_box(frame, axlist["info_radio"])
            self.DrawAntennasPlots(frame, axlist)


    def DrawAntennasPlots(self, frame, axlist):
        if self.AntennaStationID == "None": return

        for key in self.antennakeys:
            if key in frame.keys():
                if not self.AntennaStationID in [str(el) for el in frame[key].keys()]: return
                try:
                    self.TimeEfieldPlot(frame[key], axlist, 1, str(key))
                except:
                    print(key + " is not an E field key")
                try:
                    self.TimeFreqDbPlot(frame[key], axlist, 1, str(key), False)
                except:
                    print(key + " is not an waveform key")
        return

    def TimeFreqDbPlot(self, antDataMap, axlist, plotFrac, plotLabel, isADC):

        (times, hilbert1, signal1, freqs, f_signal1) = self.AntDataMapToPython(antDataMap, 0)
        (times, hilbert2, signal2, freqs, f_signal2) = self.AntDataMapToPython(antDataMap, 1)

        ax = axlist["waveforms_time"]

        self.MakeTimePlot(ax, times, signal1, plotFrac, 'b', plotLabel + " Ch.1", isADC)
        self.MakeTimePlot(ax, times, signal2, plotFrac, 'r', plotLabel + " Ch.2", isADC)
        if not isADC:
            self.MakeHilbertPlot(ax, times, hilbert1, plotFrac, 'k', plotLabel + " Ch.1", isADC)
            self.MakeHilbertPlot(ax, times, hilbert2, plotFrac, 'g', plotLabel + " Ch.2", isADC)
        ax.legend(prop={'size': 5})

        ax = axlist["waveforms_freq"]

        self.MakedBmHzPlot(ax, freqs, f_signal1, plotFrac, 'b', plotLabel + " Ch.1", isADC)
        self.MakedBmHzPlot(ax, freqs, f_signal2, plotFrac, 'r', plotLabel + " Ch.2", isADC)

        ax.legend(prop={'size': 5})

    def TimeEfieldPlot(self, vectorMap, axlist, plotFrac, plotLabel):

        times, tsX, tsY, tsZ, freqs, specX, specY, specZ = self.I3RadVector3DToPython(vectorMap)

        ax = axlist["waveforms_time"]

        self.MakeTimePlot(ax, times, tsX, plotFrac, "b", plotLabel + " Grid W", False)
        self.MakeTimePlot(ax, times, tsY, plotFrac, "r", plotLabel + " Grid N", False)
        self.MakeTimePlot(ax, times, tsZ, plotFrac, "k", plotLabel + " Vertical", False)
        ax.legend(prop={'size': 5})

        ax = axlist["waveforms_freq"]

        self.MakeFreqPlot(ax, freqs, specX, plotFrac, 'b', plotLabel + " Grid W", False)
        self.MakeFreqPlot(ax, freqs, specY, plotFrac, 'r', plotLabel + " Grid N", False)
        self.MakeFreqPlot(ax, freqs, specZ, plotFrac, 'k', plotLabel + " Vertical", False)
        ax.legend(prop={'size': 5})

    def I3RadVector3DToPython(self, vectorMap):
        for antkey in vectorMap.keys():
            # print(antkey)
            # if str([antkey.GetAntennaID(), antkey.GetStationID()]) == self.AntennaStationID:
            if str(antkey) == self.AntennaStationID:
            # if True:
                vec3d = vectorMap[antkey]
                fftData = FFTData3D()
                fftData.LoadTimeSeries(vec3d)

                timeseries = fftData.GetTimeSeries()
                spectrum = fftData.GetFrequencySpectrum()

                times = [timeseries.offset / self.timeUnit + timeseries.binning / self.timeUnit * i for i in
                         range(timeseries.GetSize())]
                freqs = [spectrum.binning / self.freqUnit * i for i in range(spectrum.GetSize())]

                tsX, tsY, tsZ = [], [], []

                for ibin in range(timeseries.GetSize()):
                    vec = timeseries[ibin]
                    tsX.append(vec.x)
                    tsY.append(vec.y)
                    tsZ.append(vec.z)

                specX, specY, specZ = [], [], []

                for ibin in range(spectrum.GetSize()):
                    vec = spectrum[ibin]
                    specX.append(abs(vec.x))
                    specY.append(abs(vec.y))
                    specZ.append(abs(vec.z))

                return times, tsX, tsY, tsZ, freqs, specX, specY, specZ

        # logging.log_fatal("(" + self.name + "): " + " The antenna with id " + str(self.antennaID) + " was not found!")
        logging.log_fatal("Nothing is plotted!")
        return 0, 0, 0, 0, 0, 0, 0, 0,  # In case things weren't found

    def AntDataMapToPython(self, antDataMap, channel_no):
        for iant, antkey in enumerate(antDataMap.keys()):
            # print(antkey)
            # if str([antkey.GetAntennaID(), antkey.GetStationID()]) == self.AntennaStationID:
            if str(antkey) == self.AntennaStationID:
            # if True:
                channelMap = antDataMap[antkey]

                channelData = channelMap[channel_no]
                channelFFT = channelData.GetFFTData()
                timeseries = channelFFT.GetTimeSeries()
                spectrum = channelFFT.GetFrequencySpectrum()
                hilbert = dataclasses.fft.GetHilbertEnvelope(channelFFT)

                times = [timeseries.offset / self.timeUnit + timeseries.binning / self.timeUnit * i for i in
                         range(timeseries.GetSize())]
                freqs = [spectrum.binning / self.freqUnit * i for i in range(spectrum.GetSize())]

                timeseriespy = []
                hilbertpy = []
                for ibin in range(timeseries.GetSize()):
                    timeseriespy.append(timeseries[ibin])
                    hilbertpy.append(hilbert[ibin])

                spectrumpy = []
                for ibin in range(spectrum.GetSize()):
                    spectrumpy.append(abs(spectrum[ibin]))

                return times, hilbertpy, timeseriespy, freqs, spectrumpy

        logging.log_warn("Data was not found!")
        return [], [], [], [], []  # In case things weren't found

    def MakeTimePlot(self, ax, times, amps, plotFrac, plotColor, plotLabel, isADC):
        plotBins = int(len(times) * plotFrac) - 1

        if not isADC:
            amps = np.array(amps)
            amps /= self.voltageUnit

        if isADC:
            mean = np.mean(amps[0:plotBins])
            ax.bar(times[0:plotBins], [i - mean for i in amps[0:plotBins]], facecolor="None", align='center',
                   edgecolor=plotColor, label=plotLabel, bottom=mean)
        else:
            rms = np.std(amps[0:plotBins])
            ax.plot(times[0:plotBins], amps[0:plotBins], color=plotColor,
                    label=plotLabel + " - RMS: {0:0.3f}".format(rms))

        if not isADC:
            thisMax = max(amps[0:plotBins])
            thisMin = min(amps[0:plotBins])
            ylow, yhigh = ax.get_ylim()
            ymax = max([ylow, yhigh, thisMax, -thisMin])
            ax.set_ylim(-ymax * 1.02, ymax * 1.02)
            ax.set_ylabel("Amplitude [" + self.voltageUnitName + "]")
        else:
            ax.set_ylabel("Amplitude [ADC]")

        ax.set_xlabel("Time [" + self.timeUnitName + "]")
        ax.set_xlim(times[0], times[plotBins])

    def MakeHilbertPlot(self, ax, times, amps, plotFrac, plotColor, plotLabel, isADC):
        plotBins = int(len(times) * plotFrac) - 1

        if not isADC:
            amps = np.array(amps)
            amps /= self.voltageUnit

        ax.plot(times[0:plotBins], amps[0:plotBins], color=plotColor, label=plotLabel, linestyle='dashed')
        # ax.plot(times[0:plotBins], amps[0:plotBins], color=plotColor, linestyle='dashed')

        if not isADC:
            ax.set_ylabel("Amplitude [" + self.voltageUnitName + "]")
        else:
            ax.set_ylabel("Amplitude [ADC]")

        ax.set_xlabel("Time [" + self.timeUnitName + "]")
        ax.set_xlim(times[0], times[plotBins])

    def MakeFreqPlot(self, ax, freqs, amps, plotFrac, plotColor, plotLabel, isADC):
        plotBins = int(len(freqs) * plotFrac) - 1

        if not isADC:
            amps = np.array(amps)
            amps /= self.specUnit

        ax.plot(freqs[1:plotBins], amps[1:plotBins], color=plotColor, label=plotLabel)
        ylow, yhigh = ax.get_ylim()
        if (yhigh >= 1000):
            ax.set_yscale("log")
        ax.set_xlabel("Frequency [" + self.freqUnitName + "]")
        ax.set_xlim(freqs[0], freqs[plotBins])

        if not isADC:
            ax.set_ylabel("Amplitude [" + self.specUnitName + "]")
        else:
            ax.set_ylabel("Amplitude [ADC/Hz]")

    def MakedBmHzPlot(self, ax, freqs, amps, plotFrac, plotColor, plotLabel, isADC):
        plotBins = int(len(freqs) * plotFrac) - 1

        amps = np.array(amps)
        df = (freqs[1] - freqs[0]) * self.freqUnit

        # Convert to power
        pz = (amps * np.sqrt(2.) * df) ** 2
        power = pz / (50 * I3Units.ohm)
        # Power per 1mW
        powerPermW = power / (1.e-3 * I3Units.joule / I3Units.second)
        # Ensure no zeros
        powerPermW += 1e-100 * min([i for i in powerPermW if i > 0])
        # dBmHz
        dBmHz = 10 * np.log10(powerPermW / df * I3Units.hertz)

        low = 1
        high = len(dBmHz)
        for ibin in range(1, len(dBmHz) - 3):
            if (0.5 * (dBmHz[ibin + 1] + dBmHz[ibin + 2]) - 0.5 * (dBmHz[ibin] + dBmHz[ibin - 1]) > 30 and dBmHz[
                ibin + 1] > -300):
                low = ibin + 1
            if (0.5 * (dBmHz[ibin + 1] + dBmHz[ibin + 2]) - 0.5 * (dBmHz[ibin] + dBmHz[ibin - 1]) < -30 and dBmHz[
                ibin] > -300):
                high = ibin

        plotFreqs = freqs[low:high + 1]
        plotAmps = dBmHz[low:high + 1]

        ax.plot(plotFreqs, plotAmps, color=plotColor, label=plotLabel)

        ylow, yhigh = ax.get_ylim()

        ax.set_xlabel("Frequency [" + self.freqUnitName + "]")
        ax.set_xlim(freqs[0], freqs[plotBins])

        ax.set_ylabel("Amplitude [dBm Hz]")

    def ToggleHidden(self):
        self.shouldDraw = not self.shouldDraw
        self.antennas_position_patches.set_visible(self.shouldDraw)
        self.antennas_pulse_patches.set_visible(self.shouldDraw)

    def antennaPatches(self, pos, x=10., y=10., rotation=0.):
        Path = mpath.Path
        off_set = 2.0
        path_data = [
            (Path.MOVETO, [off_set, off_set]),
            (Path.LINETO, [off_set, off_set+y/2]),
            (Path.LINETO, [-off_set, off_set+y/2]),
            (Path.LINETO, [-off_set, off_set]),
            (Path.LINETO, [-off_set-x/2, off_set]),
            (Path.LINETO, [-off_set-x/2, -off_set]),
            (Path.LINETO, [-off_set, -off_set]),
            (Path.LINETO, [-off_set, -off_set-y/2]),
            (Path.LINETO, [off_set, -off_set-y/2]),
            (Path.LINETO, [off_set, -off_set]),
            (Path.LINETO, [off_set+x/2, -off_set]),
            (Path.LINETO, [off_set+x/2, off_set]),
            (Path.CLOSEPOLY, [off_set, off_set])]
        codes, verts = zip(*path_data)
        path = mpath.Path(verts + pos, codes)
        patch = mpatches.PathPatch(path, edgecolor="None", facecolor=self.color, alpha=1.0)
        return patch

    def __fill_text_box(self, frame, ax):
        self.__reset_textbox(ax)
        words = ""
        # Fill the info
        words += "Antenna Station ID:\n" \
                 "{}\n".format(self.AntennaStationID)
        ax.text(0.05, 0.95, words, ha="left", va="top",
                color="k", transform=ax.transAxes)

    def __reset_textbox(self, ax):
        ax.clear()
        ax.set_xticks([])
        ax.set_yticks([])