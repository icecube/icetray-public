...
from icecube.icetray import I3Tray, i3logging
...
class TryTheScaleCalculators(icetray.I3ConditionalModule):
  ...
  def Physics(self, frame):
    ...
    if frame.Has(self.trackname):
      ...
      ## Check them!
      if ev==12:
        assert abs(scale_icetop - 5.88168) < 0.00001, "First event: mismatched IceTop scale"
        assert abs(scale_inice - 0.392260) < 0.00001, "First event: mismatched InIce scale"
        assert np.isnan(scale_infill), "First event: IceTop-Infill scale should be NAN"
        assert abs(scale_deepcore - 2.727131) < 0.00001, "First event: mismatched DeepCore scale"
      elif ev==13:
        assert abs(scale_icetop - 27.207993) < 0.00001, "Second event: mismatched IceTop scale"
        assert abs(scale_inice - 0.484603) < 0.00001, "Second event: mismatched InIce scale"
        assert np.isnan(scale_infill), "Second event: IceTop-Infill scale should be NAN"
        assert abs(scale_deepcore - 2.332320) < 0.00001, "Second event: mismatched DeepCore scale"
      elif ev==24:
        assert abs(scale_icetop - 2.815122) < 0.00001, "Third event: mismatched IceTop scale"
        assert abs(scale_inice - 0.441217) < 0.00001, "Third event: mismatched InIce scale"
        assert np.isnan(scale_infill), "Third event: IceTop-Infill scale should be NAN"
        assert abs(scale_deepcore - 1.849429) < 0.00001, "Third event: mismatched DeepCore scale"
      elif ev==27:
        assert abs(scale_icetop - 24.131528) < 0.00001, "Fourth event: mismatched IceTop scale"
        assert abs(scale_inice - 0.513447) < 0.00001, "Fourth event: mismatched InIce scale"
        assert np.isnan(scale_infill), "Fourth event: IceTop-Infill scale should be NAN"
        assert abs(scale_deepcore - 2.495409) < 0.00001, "Fourth event: mismatched DeepCore scale"
      else:
