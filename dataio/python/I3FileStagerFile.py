...
class I3FileStagerFile(AbstractFileStager):
        ...
        if parsed_url.scheme == "file":
            if parsed_url.netloc != "":
                icetray.logging.log_fatal(
                    "%s is not a valid URI (file:// URIs must use absolute paths)" % url, unit="I3FileStagerFile"
                )
            ...
            while retries > 0:
                ...
                if expected_length is not None and total_read < expected_length:
                    ...
                    if retries < 1:
                        icetray.logging.log_fatal(
                            "Error downloading %s: Expected %i bytes but got %i" % (url, expected_length, total_read),
                            unit="I3FileStagerFile",
                        )
                    else:
                        retries -= 1
                        ...
                        icetray.logging.log_warn(
                            "Error downloading %s: Expected %i bytes but got %i; %i retries remaining"
                            % (url, expected_length, total_read, retries),
                            unit="I3FileStagerFile",
                        )
                else:
                    break
    ...
    def CopyFileOut(self, local_path, url):
        ...
        if parsed_url[0] == "file":
            ...
            icetray.logging.log_fatal(
                "Can't upload to %s (you need to install the \"requests\" python library to enable this)" % url
            )
        else:
...
class SCPStager(AbstractFileStager):
    ...
    def CopyFileIn(self, url, local_path):
        ...
        if parsed.password is not None:
            icetray.logging.log_fatal(
                "Passwords in SCP URLs are not supported for security reasons; please use a key pair instead"
            )
