...
if(CMAKE_VERBOSE_MAKEFILE)
  ...
  set(DRIVER_VERBOSE False)
endif(CMAKE_VERBOSE_MAKEFILE)

#
# the programs that do the dirty work.
#
foreach(PYFILE build_slave passthru marshal start finish info post classify)
  configure_file(${BUILD_SLAVE_SOURCE_DIR}/${PYFILE}.py.in 
    ${BUILD_SLAVE_PYTHONPATH}/${PYFILE}.py 
    @ONLY
    )
endforeach(PYFILE build_slave passthru marshal start finish info post classify)

if(WIN32)
  configure_file(${BUILD_SLAVE_SOURCE_DIR}/windows_kill.py.in
    ${BUILD_SLAVE_PYTHONPATH}/kill_subprocess.py
    COPYONLY
    )
else(WIN32)
  configure_file(${BUILD_SLAVE_SOURCE_DIR}/unix_kill.py.in
    ${BUILD_SLAVE_PYTHONPATH}/kill_subprocess.py
    COPYONLY
    )
endif(WIN32)
...
#
if(BUILD_SLAVE)
  file(TO_NATIVE_PATH ${BUILD_SLAVE_PYTHONPATH}/marshal.py TEST_DRIVER)

  configure_file(${BUILD_SLAVE_SOURCE_DIR}/run_continuous_slave.py.in
...
if(BUILD_SLAVE)
  ...
  set(CMAKE_CXX_COMPILE_OBJECT 
    "\"${PYTHON_EXECUTABLE}\" \"${TEST_DRIVER}\" <CMAKE_CURRENT_BINARY_DIR> cxx_compile_object <OBJECT> ${CMAKE_CXX_COMPILE_OBJECT}" )

  set(CMAKE_CXX_CREATE_SHARED_LIBRARY  
    "\"${PYTHON_EXECUTABLE}\" \"${TEST_DRIVER}\" <CMAKE_CURRENT_BINARY_DIR> create_shared_library <TARGET> ${CMAKE_CXX_CREATE_SHARED_LIBRARY}") 
    
  set(CMAKE_CXX_CREATE_SHARED_MODULE
    "\"${PYTHON_EXECUTABLE}\" \"${TEST_DRIVER}\" <CMAKE_CURRENT_BINARY_DIR> create_shared_module <TARGET> ${CMAKE_CXX_CREATE_SHARED_MODULE}") 
    
  set(CMAKE_CXX_CREATE_STATIC_LIBRARY  
    "\"${PYTHON_EXECUTABLE}\" \"${TEST_DRIVER}\" <CMAKE_CURRENT_BINARY_DIR> create_static_library <TARGET> ${CMAKE_CXX_CREATE_STATIC_LIBRARY}") 

  set(CMAKE_CXX_LINK_EXECUTABLE  
    "\"${PYTHON_EXECUTABLE}\" \"${TEST_DRIVER}\" <CMAKE_CURRENT_BINARY_DIR> link_executable <TARGET> ${CMAKE_CXX_LINK_EXECUTABLE}") 
  ...
  #
  file(TO_NATIVE_PATH ${BUILD_SLAVE_PYTHONPATH}/start.py 
    BUILD_SLAVE_START_PY)
  add_custom_target(slave-start
    COMMAND ${PYTHON_EXECUTABLE} ${BUILD_SLAVE_START_PY}
  ...
  #
  file(TO_NATIVE_PATH ${BUILD_SLAVE_PYTHONPATH}/finish.py 
    BUILD_SLAVE_FINISH_PY)
  add_custom_target(slave-finish
    COMMAND ${PYTHON_EXECUTABLE} ${BUILD_SLAVE_FINISH_PY}
  ...
  #
  file(TO_NATIVE_PATH ${BUILD_SLAVE_PYTHONPATH}/info.py 
    BUILD_SLAVE_INFO_PY)
  add_custom_target(slave-info
    COMMAND ${PYTHON_EXECUTABLE} ${BUILD_SLAVE_INFO_PY}
...
file(TO_NATIVE_PATH ${BUILD_SLAVE_PYTHONPATH}/post.py 
  BUILD_SLAVE_POST_PY)
macro(boost_post_results PROJECT_NAME_ PARENT_TARGET BUILD_OR_TEST LOGDIR)
  if(BUILD_SLAVE)
    add_custom_command(TARGET ${PARENT_TARGET}
      POST_BUILD
      COMMAND ${PYTHON_EXECUTABLE} ${BUILD_SLAVE_POST_PY} ${PROJECT_NAME_} ${PARENT_TARGET} ${BUILD_OR_TEST} ${LOGDIR}
