...
public:
  ...
  // icecube hack:  reading in "nan" from text archives should work
    void load(double & t)
    {
        if(is.fail())
            boost::throw_exception(archive_exception(archive_exception::stream_error));
	...
	while (c == ' ' || c == '\t' || c == '\n') // munch leading whitespace
	  {
        ...
        if (c == 'n') // assume these are the only ways to write NaN
	  {
	    t = NAN;
      ...
      while (c == ' ' || c == '\t' || c == '\n') // munch leading whitespace
	{
      ...
      if (c == 'n') // danger: if NaN gets written, we have problems
	{
	  t = NAN;
	  if (is.get() != 'n' || is.get() != 'a' || is.get() != 'n') 
	    boost::throw_exception(archive_exception(archive_exception::stream_error));
