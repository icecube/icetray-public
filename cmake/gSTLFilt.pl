...
#			   This version supports the gcc 2/3 C++ compiler/library
#			   [it was developed and tested under (Mingw32) gcc 2.95.2 and 3.2]

$STLFilt_ID = "BD Software STL Message Decryptor v2.47a for gcc";

#
# (c) Copyright Leor Zolman 2002-2004. Permission to copy, use, modify, sell and
...
#       gSTLFilt.pl, keeping everyone talking despite all the curves thrown him (so far) by ActiveState
...
@nullargs = qw(boost::tuples::null_type mpl_::void_ mpl_::na);
...
while (@ARGV)									# process command-line options
{
	...
	println "Warning: unrecognized gSTLFilt.pl command line option: $ARGV[0]\n";
	shift;
}
...
@keywords = qw(algo algorithm algobase bitset cassert cctype cerrno cfloat ciso646
	...
	cstdio cstdlib cstring ctime cwchar cwctype cxxabi deque editbuf exception exception_defines
	...
	std_valarray straits strfile tempbuf alloc floatio 
 );
...
# get rid of useless messages from gcc
  ...
  if (/^(.*\.(cpp|cxx|cc|h|hpp)\:(\d+\:)? ?)/ or	#   "pathname.h:n: " or
...
# gcc set/multiset iterator:
	...
	s/[`']\b_Rb_tree_iterator ?\(($t)\)'/gen_set($1)/g;
