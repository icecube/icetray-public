            ...
            with open(csv_filename, 'r') as csvfile:
                ...
                for line_number, row in enumerate(reader):
                    ...
                    assert (
                        abs(
                            90
                            - float(row[indices[body + '_zenith [radian]']]) / I3Units.degree
                            - ephem['el'][line_number]
                        )
                        < 0.003
                    )
                    assert (
                        test_horizons.azimuth_distance(
                            90
                            - (
                                float(row[indices[body + '_azimuth [radian]']])
                                + astro.ICECUBE_LONGITUDE
                            )
                            / I3Units.degree,
                            ephem['az'][line_number],
                        )
                        < 1.0
                    )
